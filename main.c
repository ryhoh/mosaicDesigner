...
void mask(PIC design, PIC pattern, PIC *result);
int output();

int main() {
    /* 変数宣言 */
    PIC design, pattern, result;
    ...
    // fixme
    result.length = pattern.length; // 出力の画像サイズはパターンと同じにする
    
    /* データ処理 */
    adjust(&design, pattern.length);
    mask(design, pattern, &result);
...
void adjust(PIC *design, int patternLen){
    char tempGrid[GRIDSIZE][GRIDSIZE];    // 元々の模様データを退避しておく
    ...
    for(i = 0; i < patternLen; i++){
        for(j = 0; j < patternLen; j++){
            design->grid[i][j] = tempGrid[i % originalLen][j % originalLen];    // fixme
        }
    }
}
    ...
    for(i = 0; i < pattern.length; i++){
        for(j = 0; j < pattern.length; j++){
            if(pattern.grid[i][j] == 0)
                result->grid[i][j] = 0; // patternが0ならresultも0
            else
                result->grid[i][j] = design.grid[i][j]; // patternが1ならresultはdesignに従う
        }
    }
}
    ...
    for(i = 0; i < length; i++){                    // 縦のパターンの数だけループ
        for(j = 0; j < SQUARESIZE; j++){            // 小さい正方形の縦の長さだけループ
            for(k = 0; k < length; k++){            // 横のパターンの数だけループ
                for(l = 0; l < SQUARESIZE/2; l++){  // 小さい正方形の横の長さ/2だけループ
                    buff = (picture.grid[length-i-1][k] << 4) + picture.grid[length-i-1][k];  // 4bitごとに色をセット
                    if(j < FLAMETHICK || j >= SQUARESIZE-FLAMETHICK || l < FLAMETHICK || l >= SQUARESIZE-FLAMETHICK){
                        // 縁取りの対象なら
                        buff = 0b00000000;
                    }
                    fwrite(&buff, 1, 1, fp);
